...
{
        // RootHide: Do NOT create /var/jb symlink
        // Just verify jbroot is accessible
        printf("Verifying jbroot path... "); fflush(stdout); usleep(1000);
        const char *jbroot = jbclient_get_jbroot();
        if (jbroot && access(jbroot, F_OK) == 0) {
                printf("OK (%s)\n", jbroot); fflush(stdout); usleep(1000);
        }
        else {
                printf("\nWarning: jbroot not accessible\n"); fflush(stdout); usleep(1000);
        }
}
