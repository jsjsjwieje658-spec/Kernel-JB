...
int jbctl_handle_internal(const char *command, int argc, char* argv[])
{
        if (!strcmp(command, "launchd_stash_port")) {
                ...
                // Comment out protection_set_active(true) để match Dopamine-roothide fork.
                //
                // Root cause: protection_set_active(true) gọi ensure_protected() trên
                // /private/preboot/<UUID>/System và /private/preboot/<UUID>/usr, tạo
                // bindfs mounts tại đúng 2 path mà RootHide app cảnh báo:
                //   "Unknown Bindfs Mount(s):
                //    /private/preboot/.../System
                //    /private/preboot/.../usr"
                //
                // RootHide Bootstrap GỐC không bind mount /System, /usr — chỉ dùng
                // path randomization. Vì vậy RootHide app coi các bindfs này là "unknown".
                //
                // Dopamine-roothide fork (reference) đã comment out dòng này ở startup
                // (xem Dopamine-roothide/BaseBin/jbctl/src/internal.m line ~176).
                // Chúng ta làm tương tự.
                //
                // Ghi chú: protection vẫn được set 1 lần trong applyProtection() lúc
                // jailbreak đầu tiên, nhưng bind mounts KHÔNG survive userspace reboot.
                // Sau khi userspace reboot, /System và /usr sẽ KHÔNG còn bindfs →
                // RootHide app không còn cảnh báo.
                // protection_set_active(true);

                // ROOTHIDE: bootstrap launch daemons (chỉ roothide fork có)
                // Match Dopamine-roothide fork: launchctl bootstrap system launch daemons
