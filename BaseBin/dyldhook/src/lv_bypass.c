...
int HOOK(__fcntl)(int fd, int cmd, void *arg1, void *arg2, void *arg3, void *arg4, void *arg5, void *arg6, void *arg7, void *arg8)
{
        ...
        if (jbinfo_is_checked_in() || proc_has_bootstrap_port()) {
                switch (cmd) {
                        case F_ADDSIGS:
                                // In-memory signature attachment (F_ADDSIGS) is fundamentally different
                                // from file-based attachment (F_ADDFILESIGS): dyld passes a pointer to
                                // a signature blob that is already mapped in the calling process,
                                // NOT a file offset. Calling jbclient_mach_trust_file on this would
                                // try to lseek+read from `fd` at the offset stored in
                                // fsignatures_t.fs_file_start, which is meaningless for F_ADDSIGS.
                                // The Dopamine2-roothide reference implementation (44 lines) simply
                                // breaks here and lets the syscall fall through — we do the same.
                                // The previous Kernel-JB code mistakenly grouped F_ADDSIGS with
                                // F_ADDFILESIGS and called jbclient_mach_trust_file on it, which on
                                // iOS 16-17 with a developer-cert app could cause AMFI to reject the
                                // signature and SIGKILL the process.
                                break;

                        case F_ADDFILESIGS_INFO:
                                // F_ADDFILESIGS_INFO is informational-only — the kernel uses it to
                                // populate metadata about an already-trusted file, NOT to actually
                                // attach a signature. Calling jbclient_mach_trust_file on it would
                                // double-trust and could confuse AMFI's cdhash table.
                                // Fall through to the syscall return path.
                                break;
                        ...
                        case F_ADDFILESIGS_RETURN: {
                                ...
                                if (arg1) {
                                        ...
                                        if (jbinfo_should_force_cs_adhoc()) {
                                                // CS_ADHOC-stripping branch (iOS 18+ TXM / SPTM only).
                                                //
                                                // On iOS 18+, the kernel's TXM (Trust Execution Module)
                                                // requires that a library be EITHER ad-hoc-signed (CS_ADHOC)
                                                // OR have a real TeamID — having BOTH or NEITHER is rejected.
                                                // For libraries that come in with both (e.g. a developer-cert
                                                // binary that was also ad-hoc resigned by TrollStore), we
                                                // strip CS_ADHOC so TXM accepts it.
                                                //
                                                // RISK: This branch modifies the signature blob IN MEMORY
                                                // and then re-trusts it. If the cdhash of the modified
                                                // superblob does NOT match what AMFI ultimately verifies,
                                                // AMFI will SIGKILL the process. On iOS 16-17 (no TXM),
                                                // jbinfo_should_force_cs_adhoc() returns false and this
                                                // branch is skipped entirely — but on iOS 18+ with a
                                                // developer-cert app, this branch has been implicated in
                                                // random crashes when the app loads a dylib that was
                                                // ad-hoc resigned alongside a TeamID.
                                                //
                                                // MITIGATION: We keep the branch as-is for now (it matches
                                                // Dopamine2-roothide's behavior), but if you still see
                                                // dev-cert app crashes on iOS 18+, the next step is to
                                                // disable this branch entirely and let TXM fall back to
                                                // its default rejection path (which gives a cleaner error
                                                // than a SIGKILL mid-load).
