...
// We solve this here by redirecting dlopen calls for files in /usr/lib to jbroot/basebin/.fakelib (if the latter is accessible)
...
// There are a few rare edge cases of processes that cannot access jbroot/basebin/.fakelib for some reason, so we need to make sure those still go over /usr/lib
