...
// RootHide clean mode flag - when true, no jailbreak injection occurs
static bool gRootHideCleanMode = false;
...
void dyldhook_init(uintptr_t kernelParams)
{
        ...
        // Check if this process should run in clean mode (no jailbreak traces)
        // This is the core of RootHide's selective injection
        const char *cleanModeEnv = _simple_getenv(envp, ROOTHIDE_CLEAN_MODE_ENV);
        if (cleanModeEnv && strcmp(cleanModeEnv, "1") == 0) {
                gRootHideCleanMode = true;
                if (gDyldHookLog) {
                        _simple_dprintf(2, "RootHide: Process running in CLEAN MODE - no injection\n");
                }
                // In clean mode, we bail out completely before any JB initialization
                return;
        }
        
        // Also check for legacy ROOTHIDE_MODE env var
        const char *roothideModeEnv = _simple_getenv(envp, ROOTHIDE_MODE_ENV);
        if (roothideModeEnv && strcmp(roothideModeEnv, "hide") == 0) {
                gRootHideCleanMode = true;
                if (gDyldHookLog) {
                        _simple_dprintf(2, "RootHide: Process in HIDE mode - skipping injection\n");
                }
                return;
        }
        // ========== END ROOTHIDE CHECK ==========
