...
//
//  Adapted from libjailbreak/src/roothider/jailbreakd.c for use as a separate daemon binary.
//  This handles process trust patching (tree trust model):
...
int main(int argc, char* argv[]) {
    ...
    @autoreleasepool {
        // Get bootstrap port from registered ports (passed by launchd)
        ...
        // deallocate registeredPorts
        vm_deallocate(mach_task_self(), (vm_address_t)registeredPorts, registeredPortsCount * sizeof(mach_port_t));

        // Initialize jailbreak primitives (use the standard function from Kernel-JB libjailbreak)
        ...
        // Check in - get server port from launchdhook via jbclient
