...
bool should_enable_tweaks(void)
{
        ...
        //
        // Note: With the spawn-bypass in launchdhook's roothide_launchd___posix_spawn_prehook
        // (now wired up via initSpawnHooks() in spawn_hook.c), a blacklisted app should
        // NEVER even load systemhook.dylib in the first place — the spawn-bypass calls
        // __posix_spawn_orig_wrapper directly, which does NOT inject DYLD_INSERT_LIBRARIES.
        // So this `if (getenv(ROOTHIDE_CLEAN_MODE_ENV)) return false;` block is a
        // DEFENSE-IN-DEPTH for the case where:
        //   - A process was spawned via posix_spawn_hook_shared (e.g. a daemon that
        //     later execve's into a blacklisted app's binary)
        //   - The env var was inherited from a parent that was incorrectly marked
        //     clean mode.
        // In either case, we still want to skip TweakLoader.
        ...
        // ========== DYNAMIC BLACKLIST QUERY ==========
        // RootHide port (CRITICAL FIX): use isBlacklistedPath() from libjailbreak
        // (which reads Info.plist CFBundleIdentifier correctly) instead of the
        // heuristic that extracted the directory name (e.g. "IB" from "IB.app")
        // and queried the blacklist with that wrong key (config has "com.vcb.IB",
        // not "IB" → never matched).
        //
        // isBlacklistedPath(path) is defined in BaseBin/libjailbreak/src/roothider/blacklist.m
        // and:
        //   1. realpath() the executable path
        //   2. Checks it's under /private/var/containers/Bundle/Application/<36-char-UUID>/
        //   3. Reads <bundlePath>/Info.plist CFBundleIdentifier (e.g. "com.vcb.IB")
        //   4. Looks up that bundle ID in /var/mobile/Library/RootHide/RootHideConfig.plist
        //      under appconfig.<bundleID>
        //   5. Returns true if appconfig.<bundleID> == true
        //
        // If the process is a system binary (path not under /var/containers/Bundle/Application),
        // isBlacklistedPath returns false immediately without reading any plist.
        //
        // Fail-safe: if launchdhook jbserver is not up yet (early boot, in the first
        // xpcproxy before early_boot_done()), the XPC call inside isBlacklistedPath
        // may fail. In that case isBlacklistedPath returns false (treat as not
        // blacklisted) and tweaks will load — better than bootlooping.
        extern bool isBlacklistedPath(const char *path);
        if (gExecutablePath[0] != '\0' && isBlacklistedPath(gExecutablePath)) {
                // Set env var so any dylib loaded later in this process (e.g. via
                // dlopen from a tweak) also sees the clean-mode flag and skips
                // jailbreak-related work.
                setenv(ROOTHIDE_CLEAN_MODE_ENV, "1", 1);
                return false;
        }
        // ========== END DYNAMIC BLACKLIST QUERY ==========
