...
int spawnJailbreakd() {
    ...
    if (ret != 0) {
        JBLogError("posix_spawn jailbreakd failed: %d", ret);
        return ret;
    }

    JBLogDebug("jailbreakd handoff status=spawned pid=%d", pid);
...
xpc_object_t jailbreakdXpcRequest(xpc_object_t xdict) {
    ...
    if (pipe) {
        ...
        if (err != 0) {
            JBLogError("jailbreakd request failed domain=%llu action=%llu status=%d error=%s",
