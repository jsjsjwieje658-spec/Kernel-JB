...
// match Relaxin's signature. Relaxin's roothider/common.m:35 captures the
// return value (`int ret = proc_csflags_set(...)`), so the function must
// return int. Existing callers that ignore the return value are unaffected.
