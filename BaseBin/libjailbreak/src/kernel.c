...
// RootHide port (Relaxin upstream): changed return type from void to int to
// match Relaxin's signature. Returns 0 on success, -1 if the underlying
// kwrite32 fails (the existing kwrite32 helper returns int status).
int proc_csflags_update(uint64_t proc, uint32_t flags)
{
        if (gSystemInfo.kernelStruct.proc_ro.exists) {
                uint64_t proc_ro = kread_ptr(proc + koffsetof(proc, proc_ro));
                return kwrite32(proc_ro + koffsetof(proc_ro, csflags), flags);
        }
        else {
...
{
        return proc_csflags_update(proc, proc_getcsflags(proc) | (uint32_t)flags);
}
